...
describe("Logs Router", () => {
  ...
  describe("POST /logs", () => {
    ...
    it("should filter out Local Lens logs", async () => {
      const logBatch: LogBatch = {
        logs: [
          {
            timestamp: "2023-01-01T00:00:00.000Z",
          ...
          },
          {
            ...
            level: "info",
            message: "Message with local-lens content",
            pageUrl: "https://example.com",
          },
        ],
      };
  ...
  describe("GET /logs", () => {
    ...
    it("should filter by level", async () => {
      const response = await request(app).get("/logs?level=error").expect(200);
    ...
    it("should filter by URL", async () => {
      ...
      expect(response.body.logs).toHaveLength(2);
      expect(
        response.body.logs.every((log: ConsoleLog) =>
          log.pageUrl.includes("example.com")
        )
      ).toBe(true);
    });
    ...
    it("should filter by time range", async () => {
      const response = await request(app)
        .get(
          "/logs?startTime=2023-01-01T00:00:30.000Z&endTime=2023-01-01T00:01:30.000Z"
        )
        .expect(200);
    ...
    it("should handle database errors gracefully", async () => {
      ...
      logStorage.getLogs = jest
        .fn()
        .mockRejectedValue(new Error("Database error"));

      const response = await request(app).get("/logs").expect(500);
  ...
  describe("DELETE /logs", () => {
    ...
    it("should clear all logs and return count", async () => {
      const response = await request(app).delete("/logs").expect(200);
    ...
    it("should handle database errors gracefully", async () => {
      ...
      logStorage.clearLogs = jest
        .fn()
        .mockRejectedValue(new Error("Database error"));

      const response = await request(app).delete("/logs").expect(500);
  ...
  describe("GET /logs/stream", () => {
    it("should respond to stream requests", async () => {
      ...
      const promise = request(app).get("/logs/stream").timeout(100);
      ...
      try {
        ...
        expect(
          err.timeout || err.code === "ECONNABORTED" || err.status === undefined
        ).toBeTruthy();
      }
    });
  });

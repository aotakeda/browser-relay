...
describe("Integration Tests", () => {
  ...
  describe("Full Console Log Flow", () => {
    it("should handle complete console log lifecycle", async () => {
      ...
      // 2. Retrieve all logs
      const getAllResponse = await request(app).get("/logs").expect(200);
      ...
      expect(searchResponse.body.logs[0].pageUrl).toBe(
        "https://example.com/api"
      );

      // 5. Clear logs
      const clearResponse = await request(app).delete("/logs").expect(200);

      expect(clearResponse.body.cleared).toBe(2);

      // 6. Verify logs are cleared
      const emptyResponse = await request(app).get("/logs").expect(200);
  ...
  describe("Mixed Data Scenarios", () => {
    it("should handle both console logs and network requests simultaneously", async () => {
      ...
      await request(app).post("/logs").send(logBatch).expect(200);
  ...
  describe("Local Lens Filtering Integration", () => {
    it("should filter out Local Lens logs across the entire flow", async () => {
      const logBatch: LogBatch = {
        logs: [
          {
            timestamp: "2023-01-01T00:00:00.000Z",
          ...
          },
          {
            timestamp: "2023-01-01T00:01:00.000Z",
            level: "info",
            message: "[Local Lens] Debug message",
            pageUrl: "https://example.com",
          },
          {
      ...
      // Verify only non-Local Lens logs were stored
      const getResponse = await request(app).get("/logs").expect(200);
      expect(getResponse.body.logs).toHaveLength(2);

      const messages = getResponse.body.logs.map(
        (log: ConsoleLog) => log.message
      );
      expect(messages).toContain("Normal log message");
      expect(messages).toContain("Another normal log");
      expect(messages).not.toContain("[Local Lens] Debug message");
    });

    it("should filter out Local Lens network requests", async () => {
      const networkBatch: NetworkRequestBatch = {
        requests: [
          {
            requestId: "req-001",
      ...
      const getResponse = await request(app)
        ...
        .expect(200);
      expect(getResponse.body.requests).toHaveLength(1);
      expect(getResponse.body.requests[0].url).toBe(
        "https://api.example.com/data"
      );
    });

    it("should filter out requests to Local Lens server port only", async () => {
      const networkBatch: NetworkRequestBatch = {
        requests: [
          {
            requestId: "req-001",
          ...
          },
          {
            ...
            timestamp: "2023-01-01T00:01:00.000Z",
            method: "GET",
            url: "http://localhost:27497/health-local-lens",
            statusCode: 200,
            pageUrl: "https://example.com",
          },
          {
          ...
          },
          {
            ...
            method: "GET",
            url: "http://localhost:9000/health-local-lens", // Local Lens health check - should be filtered
            statusCode: 200,
            pageUrl: "https://example.com",
          },
        ],
      ...
      expect(response.body).toEqual({
        received: 5,
        stored: 2, // API request + localhost:3000 request should be stored, port 27497 + health-local-lens filtered
      });
      ...
      const storedUrls = getResponse.body.requests.map(
  ...
  describe("Error Scenarios Integration", () => {
    it("should handle invalid data gracefully across endpoints", async () => {
      ...
      expect(invalidNetworkResponse.body.error).toBe(
        "Invalid network request batch format"
      );
  ...
  describe("Performance and Limits", () => {
    it("should handle large batches efficiently", async () => {
      ...
      const getResponse = await request(app).get("/logs?limit=200").expect(200);

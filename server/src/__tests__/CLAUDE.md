# Testing Guide for Local Lens

This guide provides comprehensive documentation for writing tests in the Local Lens codebase, including standardized patterns, utilities, and best practices.
...
describe("Network Requests API", () => {
  ...
  describe("GET /network-requests", () => {
    ...
    it("should return stored requests", async () => {
      const response = await request(app).get("/network-requests").expect(200);
...
describe("Error Handling", () => {
  it("should handle invalid input gracefully", async () => {
    ...
    await expect(logStorage.insertLogs([invalidData])).rejects.toThrow(); // or specific error check
  });
...
This guide ensures all tests in the Local Lens codebase follow consistent, maintainable patterns that provide reliable test coverage and excellent developer experience.
